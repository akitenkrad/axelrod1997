...
```
results/
├── latest -> 20260415_120000                ← 最新実行へのシンボリックリンク
├── 20260415_120000/
│   ├── config.json                          ← 実行時設定
...
```
results/
├── latest -> 20260415_120500_sweep
├── 20260415_120500_sweep/
│   ├── sweep_config.json
│   └── sweep_summary.csv                    ← 全 (f, q, run) の結果
└── ...
```
...
# simulate 結果の可視化（results/latest を自動参照）
uv run python analysis/visualize.py

# 特定の実行結果を可視化
uv run python analysis/visualize.py --results_dir results/20260415_120000
...
### metrics.csv / sweep_summary.csv のカラム
